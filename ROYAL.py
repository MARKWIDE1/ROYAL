...
try:
	...
	print('[[\x1b[1;92mâ€¢\x1b[1;97m] [\x1b[1;96mS.tian_GbtðŸ’¨')
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/StianGbt/Ok/blob/main/ua2.txt').text
		ua=open('ua2.txt','w')
...
\t| |_| |/ ___ \| |_) | |_| | | |
\t \____/_/   \_\____/ \___/  |_|
   {m}°•°{k}°•°{h}°•°{sir} www.facebook.com/S.tian17 {x}{m}°•°{k}°•°{h}°•°{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}Gbt💦{x}] Masukkan Cookies :{asu} ')
...
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang [green]%s[white] Tamvan'%(my_name)))
	Stian17(f'>> Your Idz : '+str(my_id))
	Stian17(f'>> Your Ip  : {ip}')
	print('')
	...
	print('[5]Hasil Crack  ')
	print('[0]Keluar       ')
	_____Septian__Gabut_____ = input('\n>> Pilih : ')
	if _____Septian__Gabut_____ in ['1','01']:
		dump_massal()
	elif _____Septian__Gabut_____ in ['2','02']:
		dump_follower()
		...
		result()
	elif _____Septian__Gabut_____ in ['0']:
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			print(f'{k}>> Pilih Yang Bener ya!!! {x}')
			time.sleep(3)
			back()
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}â€¢{k}â€¢{h}â€¢{x} Sedang Mengetes Kesabaran {m}â€¢{k}â€¢{h}â€¢{x} <<<<< ')
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rGbtðŸ’¨ {P}[{b}{loop}{P}/{u}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
